...
export default function Butt({ svg }) {
    const arr = [`
    ...
    const fetchMyDefaultData = (e) => {
        //détermine quel bouton pour quel fetch
        //dataTitle a ajouter
        console.log(e.target.attributes.datatitle.value);
    ...
    return (
        <div onClick={fetchMyDefaultData}>{parse(arr[svg])}</div>
    )
}
